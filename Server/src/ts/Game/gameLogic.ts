...
function checkAndMovePadel(padel: string, movement: number) {
	...
	if (!currentPadel)
		return ;
	let top = parseFloat(currentPadel.style.top) || currentPadel.offsetTop;
	const paddleHeight = currentPadel.clientHeight;
	const nextPos = top + (movement * S.Objects[padel].speed);
	const newPosition = Math.max(0, Math.min(nextPos, fieldHeight - paddleHeight));
